...
class Socrata
  ...
  def initialize
    ...
    self.class.base_uri "https://#{config[:socrata][:domain]}/api"
    if config[:socrata].key?(:username) && config[:socrata].key?(:password)
      self.class.basic_auth config[:socrata][:username], config[:socrata][:password]
    end

    self.class.headers "X-App-Token" => config[:socrata][:app_token], "Content-type" => "application/json"
